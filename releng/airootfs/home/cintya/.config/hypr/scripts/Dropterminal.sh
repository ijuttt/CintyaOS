...
#
# Modified for Spotify Dropdown
# Usage: ./Dropdown_Spotify.sh [-d]
# Example: ./Dropdown_Spotify.sh
#          ./Dropdown_Spotify.sh -d (with debug output)
...
WIDTH_PERCENT=55  # Width as percentage of screen width
...
SLIDE_STEPS=1
STEP_DELAY=0.0001 # 15ms per step = ~150ms total

# Spotify command - adjust if needed
SPOTIFY_CMD="LD_PRELOAD=/usr/local/lib/spotify-adblock.so spotify"
...
calculate_dropdown_position() {
	...
	if [ $? -ne 0 ] || [ -z "$monitor_info" ]; then
		...
		echo "100 100 1000 700 fallback-monitor"
		return 1
	fi
...
# Function to get stored Spotify address
get_spotify_address() {
...
# Function to check if Spotify exists
spotify_exists() {
	local addr=$(get_spotify_address)
	if [ -n "$addr" ]; then
...
# Function to check if Spotify is in special workspace
spotify_in_special() {
	local addr=$(get_spotify_address)
	if [ -n "$addr" ]; then
...
# Function to find Spotify window by class
find_spotify_window() {
	hyprctl clients -j | jq -r '.[] | select(.class == "Spotify" or .class == "spotify") | .address' | head -1
}

# Function to spawn Spotify and capture its address
spawn_spotify() {
	debug_echo "Creating new dropdown Spotify"

	# Check if Spotify is already running
	existing_addr=$(find_spotify_window)

	if [ -n "$existing_addr" ]; then
		debug_echo "Found existing Spotify window: $existing_addr"
	...
	# Launch Spotify directly in special workspace to avoid visible spawn
	hyprctl dispatch exec "[float; size $width $height; workspace special:scratchpad silent] $SPOTIFY_CMD"

	# Wait for window to appear (Spotify might take longer to start)
	sleep 1.5

	# Find Spotify window
	local new_addr=$(find_spotify_window)

	if [ -n "$new_addr" ] && [ "$new_addr" != "null" ]; then
		# Store the address and monitor name
		echo "$new_addr $monitor_name" >"$ADDR_FILE"
		debug_echo "Spotify created with address: $new_addr in special workspace on monitor $monitor_name"
...
if spotify_exists; then
	...
	if [ "$focused_monitor" != "$dropdown_monitor" ]; then
		...
		hyprctl dispatch movewindowpixel "exact $target_x $target_y,address:$SPOTIFY_ADDR"
		hyprctl dispatch resizewindowpixel "exact $width $height,address:$SPOTIFY_ADDR"

		# Update ADDR_FILE
		echo "$SPOTIFY_ADDR $monitor_name" >"$ADDR_FILE"
	fi

	if spotify_in_special; then
		debug_echo "Bringing Spotify from scratchpad with slide down animation"
		...
		hyprctl dispatch pin "address:$SPOTIFY_ADDR"

		# Set size and animate slide down
		hyprctl dispatch resizewindowpixel "exact $width $height,address:$SPOTIFY_ADDR"
		animate_slide_down "$SPOTIFY_ADDR" "$target_x" "$target_y" "$width" "$height"

		hyprctl dispatch focuswindow "address:$SPOTIFY_ADDR"
	else
		debug_echo "Hiding Spotify to scratchpad with slide up animation"

		# Get current geometry for animation
		geometry=$(get_window_geometry "$SPOTIFY_ADDR")
		if [ -n "$geometry" ]; then
			...
			animate_slide_up "$SPOTIFY_ADDR" "$curr_x" "$curr_y" "$curr_width" "$curr_height"

			# Small delay then move to special workspace and unpin
			sleep 0.1
			hyprctl dispatch pin "address:$SPOTIFY_ADDR" # Unpin (toggle)
			hyprctl dispatch movetoworkspacesilent "$SPECIAL_WS,address:$SPOTIFY_ADDR"
		else
	...
	debug_echo "No existing Spotify found, creating new one"
	if spawn_spotify; then
		SPOTIFY_ADDR=$(get_spotify_address)
