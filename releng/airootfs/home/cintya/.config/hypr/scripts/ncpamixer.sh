...
spawn_ncpamixer() {
	...
	hyprctl dispatch exec "[float; size $width $height; workspace special:scratchpad silent] $NCPAMIXER_CMD"
...
if ncpamixer_exists; then
	...
	if ncpamixer_in_special; then
		debug_echo "Bringing ncpamixer from scratchpad with slide down animation"
		...
		debug_echo "Hiding ncpamixer to scratchpad with slide up animation"
